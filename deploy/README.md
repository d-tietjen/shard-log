...
`shard-log.service` is the hardened baseline for the open single-tenant
...
1. Install the release binary as `/usr/local/bin/shard-log-server`.
2. Create the `shard-log` system user and group.
3. Put a random token of at least 16 bytes in `/etc/shard-log/auth-token`, owned
   by `root:shard-log` with mode `0640` or stricter.
...
6. Run `systemctl daemon-reload && systemctl enable --now shard-log`.
